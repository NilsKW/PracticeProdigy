...
const STRINGS = {
  ...
  fr: {
    ...
    emptySession: "Ta file d'exercices est vide.", emptySessionSub: "Bibliothèque → appuie sur + pour ajouter des exercices, ou Favoris pour charger une séance.",
    ...
    noPresetsTitle: "Aucun favori.", noPresetsSub: "Construis une séance dans l'onglet Séance, puis reviens ici pour l'enregistrer.",
    ...
    consistency: "Continue — la régularité construit la maîtrise. 🎸",
    ...
    statsConfirmTitle: "Réinitialiser les statistiques ?", statsConfirmMsg: "Ceci supprimera définitivement tout ton historique de pratique. Cette action est irréversible.",
    statsConfirmYes: "Oui, réinitialiser", statsEmpty: "Pas encore de statistiques.", statsEmptySub: "Termine au moins un exercice pour commencer le suivi.",
    ...
    addExercisesFirst: "Ajoute d'abord des exercices à ta séance",
    ...
    badgesIntro: "Débloque des badges en atteignant des objectifs de pratique. Cette page grandira au fil des prochaines mises à jour.",
    ...
    resetBadgesConfirmTitle: "Réinitialiser tous les badges ?", resetBadgesConfirmMsg: "Ceci supprimera définitivement tous tes badges obtenus. Cette action est irréversible.",
    headerLevel: "Niveau", levelShort: (n) => `Niv. ${n}`,
    levelUnlockedTitle: "Niveau supérieur !", levelReachedLabel: (n) => `Niveau ${n}`,
    levelIntro: "Chaque minute de pratique remplit ta jauge d'expérience — monte de niveau au fil du temps.",
    levelTreeExplainer: "Cet arbre représente ta progression — il grandit et s'étoffe à mesure que tu pratiques.",
    ...
    unsavedTitle: "Modifications non enregistrées", unsavedMsg: "Tu as des modifications non enregistrées sur cet exercice. Les enregistrer avant de quitter ?",
    unsavedSaveBtn: "Enregistrer et quitter", unsavedDiscardBtn: "Quitter sans enregistrer",
    quitSessionTitle: "Quitter la séance ?", quitSessionMsg: "Tu reviendras dans l'onglet Séance pour organiser ta file d'exercices. La progression de la séance en cours s'arrête.",
    quitSessionConfirmBtn: "Quitter la séance", quitSessionCancelBtn: "Continuer",
    changelogTitle: "Quoi de neuf", changelogSubtitle: "Voici ce qui a changé depuis ta dernière visite.",
    ...
    exportBtn: "Exporter", exportingBtn: "Export en cours…", exportError: "L'export a échoué — réessaie.",
    importTitle: "Importer un groupe d'exercices", importDesc: "Choisis un fichier exporté depuis Practice Prodigy — ses catégories, exercices et fichiers sont ajoutés à ta bibliothèque.",
    ...
    aboutWhatText: "Practice Prodigy est une application pour organiser tes séances de pratique instrumentale : bibliothèque d'exercices, minuteur, métronome, suivi de progression et gamification (niveaux, badges) — quel que soit l'instrument.",
    ...
    aboutContactTitle: "Contact",
    aboutContactText: "Pour toute question, remarque ou problème, tu peux écrire directement à",
    aboutGoToFeedbackBtn: "💬 Aller à Bugs & idées",
    aboutPrivacyTitle: "⚠️ Sécurité et données",
    aboutPrivacyData: "Aucune création de compte, aucun serveur : toutes tes données (exercices, statistiques, fichiers ajoutés) restent stockées uniquement sur ton appareil, dans ton navigateur.",
    aboutPrivacyNoServer: "Rien n'est envoyé automatiquement nulle part. Les seules actions qui font sortir des données de ton appareil sont volontaires : l'envoi d'un email via « Bugs & idées », ou l'export d'un fichier via « Importer / exporter des groupes d'exercices ».",
    aboutPrivacyBackup: "Ces données ne sont pas sauvegardées ailleurs : si tu vides les données du navigateur, désinstalles l'appli ou changes d'appareil, elles peuvent être définitivement perdues si tu ne les as pas exportées au préalable.",
    aboutPrivacyDisclaimer: "Ce projet est développé de façon indépendante, sans audit de sécurité formel ni garantie de disponibilité. Évite d'y stocker des informations sensibles (données personnelles de tiers, documents confidentiels), notamment dans les fichiers pédagogiques attachés aux exercices.",
    feedbackTitle: "Signaler un bug ou proposer une idée",
    feedbackDesc: "Ouvre ton application mail avec le message prêt à envoyer au développeur.",
    feedbackBug: "Bug", feedbackIdea: "Idée",
    feedbackBugLabel: "Qu'est-ce qui n'a pas fonctionné ?", feedbackIdeaLabel: "Quelle est ton idée ?",
    feedbackBugPlaceholder: "Décris ce qui s'est passé, et ce à quoi tu t'attendais...",
    feedbackIdeaPlaceholder: "Décris ton idée...",
    feedbackSendBtn: "Envoyer par email", feedbackHint: "Ouvre ton application mail — il ne restera qu'à appuyer sur envoyer.",
  },
};
...
// ─── EXERCISE GROUP EXPORT / IMPORT ────────────────────────────────────────────
// Lets a teacher bundle a set of categories + exercises + their attached
// files into a single .zip a student downloads once and imports — no
// backend involved, everything happens client-side. JSZip is loaded lazily
// (only when export/import is actually used) from the same CDN already
// trusted for React, so it costs nothing for users who never touch this.
function loadJSZip() {
  if (window.JSZip) return Promise.resolve(window.JSZip);
  if (!loadJSZip._promise) {
    loadJSZip._promise = new Promise((resolve, reject) => {
...
// Bundles the given categories/exercises + every file they reference (looked
// up in IndexedDB by id) into one zip: manifest.json (metadata) + files/<id>
// (raw blobs). File ids are content hashes, so the exact same zip can be
// re-imported later (e.g. an updated version of a teacher's pack) and will
// cleanly overwrite the matching categories/exercises instead of duplicating.
async function exportExerciseGroup({ categories, exercises, packName }) {
  const JSZip = await loadJSZip();
  const zip = new JSZip();
  const filesFolder = zip.folder("files");
  const fileIds = new Set();
  exercises.forEach(ex => (ex.files || []).forEach(f => fileIds.add(f.id)));
  for (const id of fileIds) {
    const blob = await fileStoreGet(id);
    if (blob) filesFolder.file(id, blob);
  }
  const manifest = {
    formatVersion: 1,
    packName: packName || "Practice Prodigy",
...
// Reads a .zip produced by exportExerciseGroup: stores each attached file in
// IndexedDB — re-hashing its content rather than trusting the id in the zip,
// so file references stay correct even if the archive was altered in
// transit — then upserts the pack's categories/exercises into the caller's
// library by id (same id as an existing entry = overwrite, new id = add).
async function importExerciseGroup(file, existingExercises, existingCategories) {
  const JSZip = await loadJSZip();
  const zip = await JSZip.loadAsync(file);
  const manifestEntry = zip.file("manifest.json");
  if (!manifestEntry) throw new Error("Missing manifest.json");
  const manifest = JSON.parse(await manifestEntry.async("string"));
  if (!manifest || !Array.isArray(manifest.exercises)) throw new Error("Invalid manifest");
  ...
  for (const name of fileEntries) {
    ...
    const realId = await hashBlob(blob);
    await fileStorePut(realId, blob);
    keyToRealId[name.slice("files/".length)] = realId;
  }

  const importedExercises = manifest.exercises.map(ex => ({
    ...ex,
    files: (ex.files || []).map(f => ({ ...f, id: keyToRealId[f.id] || f.id })),
  }));
  const importedCategories = manifest.categories || [];

  const catById = new Map(existingCategories.map(c => [c.id, c]));
  importedCategories.forEach(c => catById.set(c.id, c));
  const exById = new Map(existingExercises.map(e => [e.id, e]));
  importedExercises.forEach(e => exById.set(e.id, e));

  return {
    categories: Array.from(catById.values()),
    exercises: Array.from(exById.values()),
    categoriesCount: importedCategories.length,
    exercisesCount: importedExercises.length,
...
function OnboardingTour({ onDone }) {
  ...
  const steps = [
    ...
    { icon: "📋", title: T("onboardSessionTitle"), desc: T("onboardSessionDesc") },
    { icon: "📈", title: T("onboardProgressTitle"), desc: T("onboardProgressDesc") },
    { icon: "⚙️", title: T("onboardSettingsTitle"), desc: T("onboardSettingsDesc") },
  ];
  ...
  return (
    <>
      ...
      <div style={base.scrollArea(24)}>
        ...
        {/* "À travailler" suggestions — moved to the bottom, above/instead of the top spot now taken by the add-your-own box */}
        {suggestions.length > 0 && (
          <div style={{ background: "linear-gradient(135deg,#1A1208 0%,#0F1A0F 100%)", border: "1px solid #2A2008", borderRadius: 12, padding: "12px 14px" }}>
...
// Small, short-lived confirmation meant to show near the bottom nav when an
// exercise is added to the session — a plain-word backup to the flying-icon
// animation, for anyone who doesn't catch the animation itself. Currently
// UNUSED (the flying icon alone reads clearly enough on its own) — kept
// here, ready to wire back in later, rather than deleted outright.
...
function ActiveSessionScreen({
  ...
  // Metronome: each exercise carries its own configured BPM/time signature
  // (0 = off), so switching to a different exercise — by skipping, going
  // back, finishing one naturally, or starting a session — should reset the
  // live metronome to THAT exercise's own setting, not whatever was left on
  // from the previous one. Keyed on the task's own id rather than the
  // `current` index: endSession/backToMenu/startSession all reset the index
  // to 0, so a brand-new session's first exercise would share index 0 with
  // wherever the previous session left off, and an index-only dependency
  // would miss that as "no change". Each task gets a fresh id when added to
  // a session, so this fires exactly once per real exercise switch — while
  // manual on/off or BPM/time-signature changes made mid-exercise (which
  // don't touch the task's id) are left alone until the next switch.
  useEffect(() => {
...
function SettingsScreen({ exercises, setExercises, categories, setCategories, volume, onVolumeChange, lang, onLangChange, displaySize, onDisplaySizeChange, onResetBadges, editorGuardRef, guardedRun, showChangelogOnUpdate, onShowChangelogOnUpdateChange, autoOpen, onAutoOpenConsumed }) {
  ...
  const [exportSelectedCats, setExportSelectedCats] = useState([]);
  ...
  const doExport = async () => {
    setExporting(true);
    try {
      const cats = categories.filter(c => exportSelectedCats.includes(c.id));
      const exs = exercises.filter(e => exportSelectedCats.includes(e.categoryId));
      const blob = await exportExerciseGroup({ categories: cats, exercises: exs, packName: exportName });
      const safeName = (exportName || "practice-prodigy-pack").trim().replace(/[^a-z0-9\-_ ]/gi, "").replace(/\s+/g, "-") || "practice-prodigy-pack";
      triggerDownload(blob, `${safeName}.zip`);
    } catch {
      setImportMsg({ ok: false, text: T("exportError") });
    } finally {
      setExporting(false);
    }
  };
  const handleImportFile = async e => {
    ...
    setImportMsg(null);
    try {
      const result = await importExerciseGroup(file, exercises, categories);
      setCategories(result.categories);
      setExercises(result.exercises);
      setImportMsg({ ok: true, text: T("importSuccess", result.exercisesCount, result.categoriesCount) });
    } catch {
    ...
    <div className="pp-narrow" style={base.scrollArea(24)}>
      ...
      {section === "exercises" && (
        <>
          ...
          {(() => {
            // Exercises whose categoryId no longer matches any existing
            // category (e.g. the category they were created under was later
            // deleted) — the Library still shows these under "Unknown", but
            // until now this list only ever looped over `categories`, so
            // they were invisible here and impossible to edit, reassign, or
            // delete. Surface them the same way Library does.
            const orphanExs = exercises.filter(e => !categories.some(c => c.id === e.categoryId));
            if (orphanExs.length === 0) return null;
            return (
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "#AEB0C0", padding: "4px 0 6px", fontWeight: 700 }}>{T("unknownCategory")}</div>
                <div style={base.card}>
                  {orphanExs.map((ex, idx) => (
                    <div key={ex.id} style={{ ...base.row, borderBottom: idx < orphanExs.length - 1 ? `1px solid ${C.faint}` : "none", cursor: "pointer" }} onClick={() => setEditEx(ex)}>
                      <span style={{ fontSize: 16 }}>{ex.icon}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 600 }}>{exerciseName(ex, lang)}</div>
                      </div>
      ...
      {section === "share" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={base.card}>
            <div style={{ padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
                {categories.map(cat => (
                  <label key={cat.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: C.cream, cursor: "pointer" }}>
                    <input type="checkbox" checked={exportSelectedCats.includes(cat.id)} onChange={() => toggleExportCat(cat.id)} />
                    <span style={{ width: 10, height: 10, borderRadius: "50%", background: cat.color, flexShrink: 0 }} />
                    <span style={{ flex: 1 }}>{categoryName(cat, lang)}</span>
                    <span style={{ fontSize: 11, color: C.muted }}>{T("exercisesCount", exercises.filter(e => e.categoryId === cat.id).length)}</span>
                  </label>
                ))}
              </div>
          ...
          <div style={base.card}>
            <div style={{ padding: "14px 16px" }}>
              <label style={{ ...base.label, margin: 0 }}>{T("importTitle")}</label>
              <div style={{ fontSize: 11, color: C.muted, marginTop: 4, marginBottom: 12 }}>{T("importDesc")}</div>
              <label style={{ ...base.pillBtn(false), textAlign: "center", display: "block", cursor: "pointer" }}>
                {importing ? T("importingBtn") : T("importBtn")}
                <input type="file" accept=".zip,application/zip" style={{ display: "none" }} onChange={handleImportFile} disabled={importing} />
              </label>
              {importMsg && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, flexShrink: 0 }}>
          <div style={{ background: "#1A1A1A", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "flex-start", gap: 8 }}>
            ...
            <span style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5 }}>
              Cette section sert au développement de l'application — elle n'a aucune utilité pour ton usage de l'appli.
            </span>
          </div>
          ...
          {debugSubSection === "manual" && (
            <>
              ...
              <div style={{ fontSize: 11, color: C.muted, padding: "0 4px" }}>
                Aperçu de ce qui remplacera plus tard l'écran « Niveau ». Les curseurs ci-dessous servent uniquement à tester le rendu — ils n'utilisent pas tes vraies statistiques.
              </div>
...
// Deterministic pseudo-random in [0,1) from a string seed (FNV-1a-ish hash).
// Using each node's own key path (e.g. "b2-1-0") as the seed means the same
// tree config always renders identically — no flicker on re-render — while
// still varying naturally from one branch/sub-branch to the next, unlike
// Math.random() (different every render) or a fixed formula (identical
// pattern repeated at every branch).
...
function buildBranchNode({ attachX, attachY, dirAngle, len, color, depth, key }, opts) {
  ...
  if (depth < opts.maxDepth && opts.subCount > 0 && opts.counter.n < TREE_MAX_NODES) {
    ...
    // Every rotation term above is directly proportional to the bias slider
    // with no independent baseline, and each level adds its own on top of
    // whatever its parent already accumulated. For a node whose own random
    // lean happens to point the same way as its parent's, that compounds
    // — 2 levels deep it's already the sum of 2 full-strength turns, 4 levels
    // deep the sum of 4 — so raising the bias visibly "winds up" the branches
    // near the tip like a spring, while the base (a single, undamped turn)
    // barely seems to move by comparison. Damping each level's contribution
    // by how deep it already is keeps that compounding bounded: the first
    // split still turns at full strength, but each further nested split
...
export default function App() {
  ...
  //
  // `suppressAutoFlashRef` + `triggerSessionFlash` are kept generic/reusable
  // on purpose: any future action that wants the flash to land at a specific
  // moment (e.g. timed to another animation) rather than instantly can set
  // the ref to true right before its own state change, then call
  // triggerSessionFlash() itself whenever it actually wants the tab to light
  // up — exactly what addExerciseWithFlight below does for the flying icon.
  ...
  // actually went somewhere, which the red flash alone didn't convey. The
  // nav tab's flash is deliberately delayed (see SESSION_FLASH_DELAY_S) to
  // land as the icon "arrives" rather than firing the instant it takes off.
  const sessionNavRef = useRef(null);
  const [flyingItems, setFlyingItems] = useState([]);
  const addExerciseWithFlight = (ex, fromRect) => {
    suppressAutoFlashRef.current = true;
    addExercise(ex);
    setTimeout(triggerSessionFlash, SESSION_FLASH_DELAY_S * 1000);
